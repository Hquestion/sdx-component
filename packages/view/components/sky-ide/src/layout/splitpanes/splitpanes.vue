...
export default {
    ...
    'methods': {
        ...
        update() {
            ...
            [...this.container.children].forEach((child) => {
                if (child.classList.contains('splitpanes__pane')) { // Pane
                    ...
                    const preVM = paneIndex > 0 ? this.$children[paneIndex - 1] : null;
                    const nextVM = paneIndex + 1 >= this.container.children.length ? null : this.$children[paneIndex + 1];

                    if (lastIsPane && vm.visible) { // The previous child is a pane: we need to create a new splitter in between.
                    ...
                    if (!setPanesSizesToDefault) {
                        if (typeof vm.size === 'undefined') {
                            ...
                            if (!vm.visible) {
                                child.setAttribute('data-origin-size', vm.size);
                                size = 0;
                                // if (preVM) {
                                //     preVM.size = Number.parseFloat(preVM.size) + Number.parseFloat(vm.size);
                                // } else if (nextVM) {
                                //     nextVM.size = Number.parseFloat(nextVM.size) + Number.parseFloat(vm.size);
                                // }
                            } else {
                                // The size is given in prop.
                                size = Number.parseFloat(vm.size);
                                if (preVM && !preVM.visible) {
                                    size = size + Number.parseFloat(preVM.size);
                                }
                                if (nextVM && !nextVM.visible) {
                                    size = size + Number.parseFloat(nextVM.size);
                                }
                            }
                        }
                    }
        ...
        requestUpdateVisible({target, ...args}) {
            const index = +target.$el.getAttribute('data-splitpanes-index');
            const originSize = target.$el.getAttribute('data-origin-size');
            const pane = this.panes[index];
            Object.entries(args).forEach(([key, value]) => {
                pane[key] = value;
            });
            const prePane = index > 0 ? this.panes[index - 1] : null;
            const nextPane = this.panes[index + 1] || null;
            if (pane.visible) {
                if (originSize && originSize !== pane.size) {
                    pane.size = Number.parseFloat(originSize);
                }
                if (prePane) {
                    prePane.size = prePane.size - pane.size;
                } else if (nextPane) {
                    nextPane.size = nextPane.size - pane.size;
                }
            } else {
                target.$el.setAttribute('data-origin-size', pane.size);
                if (prePane) {
                    prePane.size = prePane.size + pane.size;
                } else if (nextPane) {
                    nextPane.size = nextPane.size + pane.size;
