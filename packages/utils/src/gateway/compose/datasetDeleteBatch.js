...
export let handler = wrap(function(ctx, request) {
    ctx.debug('Dataset DELETE BODY ' + request.Body);
    const requests = JSON.parse(request.Body).uuids.map(uuid => ctx.createDeleteRequest(`http://tyk-gateway/data-manager/api/v1/datasets/${uuid}`));
